...
def unoverlay(project, path, ignore_error=False):
    image_dir = project.image_dir()
    run('/bin/umount -R {image_dir}/{path}'.format(path=path, image_dir=image_dir), ignore_error)
...
def mkinitcpio(project):
    try:
        ...
        run('/bin/mount --bind /dev %s/dev' % image_dir)
        # overlay(project, "dev") # yemedi nedense
...
def setup_grub(project):
    ...
    for name in os.listdir(path):
        ...
        if name.startswith("kernel") or name.startswith("initramfs") or name.startswith("initr") or name.endswith(".bin"):
            if name.startswith("kernel"):
                kernel = name
...
def generate_isolinux_conf(project):
    ...
    if "mkinitcpio" in project.all_install_image_packages:
        ...
        # dict['exparams'] += " misobasedir=pisi misolabel=Pisi_Linux_2.4.2_KDE6 overlay=free "
...
label %(title)s
    kernel /pisi/boot/kernel
    append initrd=/pisi/boot/initrd 2026-04-30.uuid %(exparams)s
...
#        image_file = project.image_file()
        ...
        # f.write("liveroot=LABEL=Pisi_Linux_2.4.2_KDE6\n")
        f.close()
...
def load_grub_params(project, initcpio=False):
    ...
    try:
        if initcpio:
            ...
            # initcpio = 'options misobasedir=pisi misolabel=Pisi_Linux_2.4.2_KDE6 overlay=free '
        else:
            initcpio = ''
...
def make_iso(project, toUSB=False, dev="/dev/sdc1"):
    ...
    try:
        ...
        run("mkdir %s/boot" % iso_dir)
        run("touch %s/boot/2026-04-30.uuid" % iso_dir)
        ...
        # -uid 0 -gid 0 -udf -allow-limited-size -iso-level 3 -full-iso9660-filenames -joliet-long -rational-rock \
        # -input-charset utf-8 -no-emul-boot -boot-load-size 4 -isohybrid-gpt-basdat \
        ...
        -preparer "prepared by mkpisiiso" \
        -publisher "%s" -A "%s"  %s' % (
            label, iso_file, iso_dir, iso_dir, publisher, application, iso_dir)
                    # orj
        # the_iso_command = 'xorriso -as mkisofs \
